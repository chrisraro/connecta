...
export function DigitalBusinessCard({
  ...
  if (activeTheme === "light") {
    ...
    if (bgType === "solid") {
      const baseColor = config?.backgroundColor
        ? resolveColor(config.backgroundColor, "#ffffff")
        : "#ffffff";
    ...
    if (bgType === "solid") {
      const baseColor = config?.backgroundColor
        ? resolveColor(config.backgroundColor, "#0d0d0d")
        : "#0d0d0d";
  ...
  const handleDragStart = (
    ...
    elementKey: "header" | "qr" | "bio" | "contacts",
  ) => {
    ...
    const handleDrag = (moveEvent: MouseEvent | TouchEvent) => {
      ...
      const newX = Math.max(
        0,
        Math.min(100 - (initialPos.width || 20), Math.round(initialPos.x + deltaX)),
      );
  ...
  const handleResizeStart = (
    ...
    const handleResize = (moveEvent: MouseEvent | TouchEvent) => {
      ...
      const newWidth = Math.max(
        10,
        Math.min(100 - initialPos.x, Math.round(initialWidth + deltaX)),
      );
      ...
      const newScale = Math.max(
        0.4,
        Math.min(2.5, Math.round((initialScale + scaleChange) * 100) / 100),
      );
        ...
        <div className="space-y-1 text-[9px] font-medium min-w-0">
          {allPhones.map((ph, i) => (
            <div key={`phone-${i}`} className="flex items-center gap-1.5 opacity-90 truncate">
              <Phone
                style={{ color: resolvedTextColor }}
                className="w-2.5 h-2.5 shrink-0 opacity-85"
              />
              <span>{ph}</span>
            </div>
          ))}
