...
export default function AdminUsersPage() {
    ...
    const handleGrant = (targetUserId: Id<"users">, role: "superadmin" | "moderator") =>
        run(targetUserId, () =>
            grantAdminRole({ adminClerkId: user!.id, targetUserId, role })
        );

    const handleRevoke = (targetUserId: Id<"users">) => {
        if (!confirm("Revoke admin access for this user?")) return;
        run(targetUserId, () =>
            revokeAdminRole({ adminClerkId: user!.id, targetUserId })
        );
    };
    ...
    const handleSuspend = (targetUserId: Id<"users">, suspended: boolean) => {
        ...
        run(targetUserId, () =>
            setUserSuspended({ adminClerkId: user!.id, targetUserId, suspended })
        );
    };
                                    ...
                                    <TableRow key={u.id} className="border-border hover:bg-muted/50 transition-colors">
                                        ...
                                        <TableCell>
                                            {isSuspended ? (
                                                ...
                                                <Badge className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 hover:bg-emerald-500/20">Onboarded</Badge>
                                            ) : (
                                                <Badge variant="secondary" className="bg-amber-500/10 text-amber-500 hover:bg-amber-500/20 border-amber-500/20">Pending Setup</Badge>
                                            )}
                                        </TableCell>
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end" className="bg-card border-border text-foreground">
                                                        ...
                                                        {u.role === "admin" ? (
                                                            ...
                                                            <>
                                                                <DropdownMenuItem onClick={() => handleGrant(u.id as Id<"users">, "moderator")}>
                                                                    Grant moderator
                                                                </DropdownMenuItem>
                                                                <DropdownMenuItem onClick={() => handleGrant(u.id as Id<"users">, "superadmin")}>
                                                                    Grant superadmin
                                                                </DropdownMenuItem>
                                                            </>
                                                        )}
