  ...
  const handleStatusUpdate = async (orderId: Id<"orders">, newStatus: string) => {
    ...
    try {
      ...
      setSelectedOrder((prev: Doc<"orders"> | null) =>
        prev ? { ...prev, status: validStatus } : prev,
      );
    } catch (error) {
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-start gap-3 p-3 rounded-lg border border-amber-600/30 bg-amber-600/5 text-sm">
                ...
                <p className="text-muted-foreground">
                  Marking an order refunded here restores inventory and records the refund, but the
                  actual money refund must be issued in the{" "}
                  <a
                ...
                <CardContent className="pt-6 space-y-2">
                  ...
                  {(selectedOrder.discount ?? 0) > 0 && (
                    <div className="flex justify-between text-green-400">
                      <span>
                        Discount
                        {selectedOrder.appliedDiscountCode
                          ? ` (${selectedOrder.appliedDiscountCode})`
                          : ""}
                      </span>
