...
export default function AdminFactoryPage() {
  ...
  const startScanning = async () => {
    ...
    if (!nfcHost) {
      setScanError("Card writing is unavailable: no NFC host is configured for this deployment.");
      return;
    }
    ...
    try {
      ...
      ndef.onreading = async (event: NDEFReadingEvent) => {
        ...
        try {
          ...
          await withRetries(() => ndef.write({ records: [{ recordType: "url", data: url }] }), {
            attempts: 3,
            delayMs: 250,
          });
      ...
      {!nfcHost && (
        <Alert variant="destructive" className="mb-8" id="nfc-host-warning">
          ...
          <AlertDescription>
            ...
            <code className="font-mono">NEXT_PUBLIC_APP_URL</code> and redeploy before scanning or
            printing labels — tags written without it would point nowhere, and once shipped they
            can&apos;t be repointed.
          </AlertDescription>
        </Alert>
      )}
      ...
      {isLocalNfcHost && (
        <Alert className="mb-8" id="nfc-local-host-warning">
          ...
          <AlertDescription>
            ...
            Cards encoded while this is set will only open correctly on this computer — set{" "}
            <code className="font-mono">NEXT_PUBLIC_APP_URL</code> to a public deployment before
            writing tags meant to leave it.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="mb-8 bg-card/50 border border-border rounded-3xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-background/50 rounded-2xl p-4 border border-border">
            ...
            <p className="text-xs text-muted-foreground">
              After signup, card is automatically claimed and linked to their profile.
            </p>
          </div>
        </div>
      </div>
                          ...
                          <div className="flex items-center justify-end gap-1">
                            <Button
                              ...
                              title={
                                nfcHost
                                  ? "Print Sticker"
                                  : "No NFC host configured — see the notice above."
                              }
                              aria-describedby={!nfcHost ? "nfc-host-warning" : undefined}
                            >
