...
function GalleryUploader({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSizeMB * 1024 * 1024) {
      toast.error(
        `Image must be under ${maxSizeMB}MB. Your file is ${(file.size / (1024 * 1024)).toFixed(1)}MB.`,
      );
      return;
    }
...
function BuilderContent() {
  ...
  // New field states
  const [certification, setCertification] = useState<{ title: string; description: string }>({
    title: "",
    description: "",
  });
  ...
  const [newPropertyListing, setNewPropertyListing] = useState<{
    title: string;
    description: string;
    price: string;
    location: string;
    status: string;
    link: string;
  }>({ title: "", description: "", price: "", location: "", status: "for-sale", link: "" });

  // Inline Projects state
  const [inlineProjects, setInlineProjects] = useState<InlineProject[]>([]);
  const [newInlineProject, setNewInlineProject] = useState<{
    title: string;
    description: string;
    category: string;
    link: string;
  ...
  useEffect(() => {
    ...
    try {
      if (editingId && existingProfile) {
        ...
        if (info.experience)
          setExperience(info.experience.map((e) => ({ ...e, description: e.description || "" })));
        if (info.testimonials)
          setTestimonials(info.testimonials.map((t) => ({ ...t, role: t.role || "" })));
  ...
  const handleProfileTypeChange = (type: ProfileType) => {
    ...
    const matchingTemplate =
      type === "business" ? "architectural" : type === "company" ? "kinetic" : "editorial";
    ...
    setBlocks((prev) =>
      prev.map((block) => {
        if (type === "individual") {
          if (
            ["Hero", "About", "Contact", "Projects", "Experience", "Education"].includes(block.id)
          ) {
          ...
          if (
            ["Hero", "About", "Contact", "Services", "Products", "Properties", "Gallery"].includes(
              block.id,
            )
          ) {
            return { ...block, isEnabled: true };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { componentOrder, filteredAgentInfo: filteredCleanAgentInfo } =
        deriveBuilderProfileFields(profileType, blocks, cleanAgentInfo);
      ...
      const { id: profileId, slug } = await createProfile({
        ...
        layoutConfig: {
          ...
          colorPalette: {
            ...
            secondary:
              customColors.secondary !== customColors.primary ? customColors.secondary : undefined,
  ...
  const renderPreview = () => {
    const { componentOrder, filteredAgentInfo } = deriveBuilderProfileFields(profileType, blocks, {
      ...agentInfo,
      certification,
      education,
      techStack,
            ...
            <>
              ...
              <div className="px-4 py-4">
                ...
                {showReorderMode ? (
                  <div className="space-y-2">
                    ...
                    >
                      <SortableContext
                        items={getBlocksForProfileType(profileType, blocks)}
              ...
              >
                <div className="space-y-4">
                  <div>
                    <Label htmlFor={`${uid}-fullName`} className="text-sm">
                      Full Name
                    </Label>
                    <Input
                      id={`${uid}-fullName`}
                  ...
                  <div>
                    <Label htmlFor={`${uid}-jobTitle`} className="text-sm">
                      Title
                    </Label>
                    <Input
                      id={`${uid}-jobTitle`}
                  ...
                  <div>
                    <Label htmlFor={`${uid}-company`} className="text-sm">
                      Company
                    </Label>
                    <Input
                      id={`${uid}-company`}
                  ...
                  <div>
                    <Label htmlFor={`${uid}-phone`} className="text-sm">
                      Phone
                    </Label>
                    <Input
                      id={`${uid}-phone`}
                  ...
                  <div>
                    <Label htmlFor={`${uid}-website`} className="text-sm">
                      Website
                    </Label>
                    <Input
                      id={`${uid}-website`}
                    ...
                    <div className="space-y-2 mt-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            const platform = (
                              document.getElementById("social-platform") as HTMLSelectElement
                            ).value;
                            const url = (document.getElementById("social-url") as HTMLInputElement)
                              .value;
                            if (platform && url) {
                              setAgentInfo({
                                ...agentInfo,
              ...
              >
                <div className="space-y-4">
                  <div>
                    <Label htmlFor={`${uid}-about`} className="text-sm">
                      About Me
                    </Label>
                    <textarea
                      id={`${uid}-about`}
              ...
              >
                <div className="space-y-4">
                  <div>
                    <Label htmlFor={`${uid}-cert-title`} className="text-sm">
                      Title
                    </Label>
                    <Input
                      id={`${uid}-cert-title`}
                  ...
                  <div>
                    <Label htmlFor={`${uid}-cert-desc`} className="text-sm">
                      Description
                    </Label>
                    <textarea
                      id={`${uid}-cert-desc`}
                      ...
                      onChange={(e) =>
                        setCertification({ ...certification, description: e.target.value })
                      }
                      placeholder="Description..."
                    />
                  </div>
              ...
              >
                ...
                >
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setNewInlineProject({ ...newInlineProject, description: e.target.value })
                    }
                  />
                  <select
                    ...
                    onChange={(e) =>
                      setNewInlineProject({ ...newInlineProject, category: e.target.value })
                    }
                  ...
                  <Input
                    aria-label="External URL (optional)"
                    placeholder="External URL (optional)"
                    value={newInlineProject.link}
                    onChange={(e) =>
                      setNewInlineProject({ ...newInlineProject, link: e.target.value })
                    }
                  />
                </EditableList>
              </InspectorPanel>
              ...
              >
                ...
                >
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setNewPropertyListing({ ...newPropertyListing, description: e.target.value })
                    }
                  />
                  <Input
                    aria-label="Price"
                    placeholder="Price (e.g., $250,000)"
                    value={newPropertyListing.price}
                    onChange={(e) =>
                      setNewPropertyListing({ ...newPropertyListing, price: e.target.value })
                    }
                  />
                  <Input
                    aria-label="Location"
                    placeholder="Location"
                    value={newPropertyListing.location}
                    onChange={(e) =>
                      setNewPropertyListing({ ...newPropertyListing, location: e.target.value })
                    }
                  />
                  <select
                    ...
                    onChange={(e) =>
                      setNewPropertyListing({ ...newPropertyListing, status: e.target.value })
                    }
                    className="min-h-11 lg:min-h-0 w-full h-10 rounded-lg border border-border bg-background px-3 text-sm"
                  >
                  ...
                  <Input
                    aria-label="Listing URL (optional)"
                    placeholder="Listing URL (optional)"
                    value={newPropertyListing.link}
                    onChange={(e) =>
                      setNewPropertyListing({ ...newPropertyListing, link: e.target.value })
                    }
                  />
                </EditableList>
              </InspectorPanel>
              ...
              <div className="px-4 py-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label
                      htmlFor={`${uid}-color-background-hex`}
                      className="text-xs text-muted-foreground mb-1.5 block"
                    >
                      Background
                    </label>
                  ...
                  <div>
                    <label
                      htmlFor={`${uid}-color-secondary-hex`}
                      className="text-xs text-muted-foreground mb-1.5 block"
                    >
                      Secondary
                    </label>
