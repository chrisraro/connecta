 ...
 * record label.
 */
function slugSourceFor(profile: { name: string; agentInfo?: { fullName?: string } }): string {
...
export const internalBackfillSlugs = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const page = await ctx.db.query("profiles").paginate({ cursor: args.cursor ?? null, numItems });
