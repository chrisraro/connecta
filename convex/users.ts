...
export const internalStripLegacyCredits = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const page = await ctx.db.query("users").paginate({ cursor: args.cursor ?? null, numItems });
...
export const deleteMyAccount = action({
  ...
  handler: async (
    ...
    const erasure = await ctx.runMutation(internal.users.internalEraseUserByClerkId, { clerkId });
    ...
    let res: Response;
    try {
      res = await fetch(`https://api.clerk.com/v1/users/${encodeURIComponent(clerkId)}`, {
        method: "DELETE",
