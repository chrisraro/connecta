...
- **Card skins and plans (confirmed 2026-09-24):** one default skin (Charcoal) is free; **every other skin is subscription-only**, both in print and as the owner's digital card. Enforced in the builder UI today (`lib/plans.ts` `allowedCardSkins`); no database check re-enforces it yet **(open)**. What a free owner's card shows after a subscription lapses is **(open)**.
- **Card skin roadmap (confirmed 2026-09-24):** more skins and colourways are coming, and a **portrait orientation** alongside today's landscape one. Adding an orientation or new skins needs a new `card_skin` enum value (and possibly an orientation column); which skins, their names, and whether portrait is per-skin or a separate choice are **(open)**. The digital card (`components/ui/digital-business-card.tsx`) and every skin's artwork (`lib/cardSkins.ts`) are the one source for screen and print.
