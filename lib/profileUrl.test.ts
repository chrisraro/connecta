...
test("profileUrl joins an origin with the resolved path", () => {
  expect(profileUrl("https://sigmatap.example", { slug: "jane-doe", _id: "xyz" })).toBe(
    "https://sigmatap.example/jane-doe"
  );
});

test("profileUrl strips a trailing slash from the origin before joining", () => {
  expect(profileUrl("https://sigmatap.example/", { slug: undefined, _id: "xyz" })).toBe(
    "https://sigmatap.example/p/xyz"
  );
});
